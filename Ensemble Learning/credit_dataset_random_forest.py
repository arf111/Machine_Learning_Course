...
rf = RandomForest(train_data, test_data,
                  list(train_data.columns[:-1]),
                  train_data['y'], 16, T, 2)
...
fig1 = plt.figure(1)
ax1 = plt.axes()
ax1.plot(x, rf.train_error, c='b', label='Train Error')
ax1.plot(x, rf.test_error, c='r', label='Test Error')
ax1.set_title("Random Forest Error")
plt.legend()
plt.savefig("random_forest_credit.png")
plt.show()
