...
for _ in tqdm(range(100)):
    index = np.random.choice(np.arange(len(preprocessed_bank_train_df)), size=1000, replace=False)
    rf = RandomForest(preprocessed_bank_train_df, preprocessed_bank_test_df,
...
bias_random_forest = bias_squared(preprocessed_bank_test_df['y'], y_pred_bagged_tree)
variance_single_tree = variance(y_pred_single_tree)
variance_random_forest = variance(y_pred_bagged_tree)

print("Bias of single tree: ", bias_single_tree)
print("Bias of bagged tree: ", bias_random_forest)
print("Variance of single tree: ", variance_single_tree)
print("Variance of bagged tree: ", variance_random_forest)
print("Total error of single tree: ", bias_single_tree + variance_single_tree)
print("Total error of bagged tree: ", bias_random_forest + variance_random_forest)

f = open("random_forest_2e.txt", "a")
f.write(f"Bias of single tree:  {bias_single_tree}\n")
f.write(f"Bias of bagged tree:  {bias_random_forest}\n")
f.write(f"Variance of single tree:  {variance_single_tree}\n")
f.write(f"Variance of bagged tree:  {variance_random_forest}\n")
f.write(f"Total error of single tree:  {bias_single_tree + variance_single_tree}\n")
f.write(f"Total error of bagged tree:  {bias_random_forest + variance_random_forest}\n")
f.close()
